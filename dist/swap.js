...
exports.getNetworkCongestion = getNetworkCongestion;
exports.swapBuy = swapBuy;
exports.swapSell = swapSell;
exports.getTokenPrice = getTokenPrice;
exports.getTokenBalance = getTokenBalance;
exports.getSolBalance = getSolBalance;
exports.getRecentFeesSpent = getRecentFeesSpent;
...
const WS_URL = process.env.WS_URL ||
    (currentEndpoint === null || currentEndpoint === void 0 ? void 0 : currentEndpoint.replace("https://", "wss://").replace(/\/$/, ""));
let connection = new web3_js_1.Connection(currentEndpoint, { wsEndpoint: WS_URL });
...
function switchRpcEndpoint() {
    ...
    if (newEndpoint) {
        ...
        connection = new web3_js_1.Connection(newEndpoint, { wsEndpoint });
...
let cachedBlockhashData = null;
// Cache for network congestion data
let congestionCache = null;
...
function setupBlockhashSubscription() {
    connection.onSlotChange((slotInfo) => __awaiter(this, void 0, void 0, function* () {
        try {
            const blockhashData = yield connection.getLatestBlockhash({ commitment: "finalized" });
            cachedBlockhashData = Object.assign(Object.assign({}, blockhashData), { timestamp: Date.now() });
...
function getLatestBlockhash() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!cachedBlockhashData || Date.now() - cachedBlockhashData.timestamp > 5000) {
            try {
                const newBlockhashData = yield connection.getLatestBlockhash({ commitment: "finalized" });
                cachedBlockhashData = Object.assign(Object.assign({}, newBlockhashData), { timestamp: Date.now() });
...
function getNetworkCongestion() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const perfSamples = yield connection.getRecentPerformanceSamples(5);
            ...
            let factor = 1.0;
            if (avgTxPerSlot > 4500)
                factor = 6.0;
            else if (avgTxPerSlot > 3500)
                factor = 4.5;
            else if (avgTxPerSlot > 2500)
                factor = 3.0;
            else if (avgTxPerSlot > 1500)
                factor = 2.0;
            else if (avgTxPerSlot > 800)
                factor = 1.5;
 ...
 */
function axiosRequestWithBackoff(config_1) {
    return __awaiter(this, arguments, void 0, function* (config, maxRetries = 5) {
        var _a, _b;
        let retries = 0;
        while (true) {
            try {
                ...
                if ((error.response && error.response.status === 429 ||
                    ...
                    retries < maxRetries) {
                    const retryAfter = parseInt(((_b = (_a = error.response) === null || _a === void 0 ? void 0 : _a.headers) === null || _b === void 0 ? void 0 : _b["retry-after"]) || "0", 10) * 1000;
 ...
 */
function confirmTransactionWithRetry(signature_1, blockhash_1, lastValidBlockHeight_1) {
    return __awaiter(this, arguments, void 0, function* (signature, blockhash, lastValidBlockHeight, maxRetries = 3) {
        var _a, _b, _c, _d;
        ...
        const finalStatus = yield connection.getSignatureStatus(signature);
        return ((_c = finalStatus === null || finalStatus === void 0 ? void 0 : finalStatus.value) === null || _c === void 0 ? void 0 : _c.confirmationStatus) === 'confirmed' ||
            ((_d = finalStatus === null || finalStatus === void 0 ? void 0 : finalStatus.value) === null || _d === void 0 ? void 0 : _d.confirmationStatus) === 'finalized';
    });
}
/**
 ...
 */
function swapBuy(tokenAddress_1, amount_1) {
    return __awaiter(this, arguments, void 0, function* (tokenAddress, amount, priorityFeeMultiplier = 1, retryCount = 0, slippageOverride = null) {
        const MAX_RETRIES = 12;
        const MAX_FEE_MULTIPLIER = 15;
        try {
            // Get fresh blockhash
            const { blockhash, lastValidBlockHeight } = yield getLatestBlockhash();
            // Get priority fee data with retry
            let baseFee = 1000000; // Default high value in case API fails
            try {
                const feeUrl = `${raydium_sdk_v2_1.API_URLS.BASE_HOST}${raydium_sdk_v2_1.API_URLS.PRIORITY_FEE}`;
                const feeResponse = yield axiosRequestWithBackoff({ method: "get", url: feeUrl });
            ...
            // Get swap quote
            const swapQuoteUrl = `${raydium_sdk_v2_1.API_URLS.SWAP_HOST}/compute/swap-base-in?inputMint=${spl_token_1.NATIVE_MINT}&outputMint=${tokenAddress}&amount=${amount}&slippageBps=${effectiveSlippage * 100}&txVersion=V0`;
            const swapQuoteResponse = yield axiosRequestWithBackoff({ method: "get", url: swapQuoteUrl });
            const swapResponse = swapQuoteResponse.data;
            // Request transaction with higher compute limit
            const swapTxUrl = `${raydium_sdk_v2_1.API_URLS.SWAP_HOST}/transaction/swap-base-in`;
            const swapTxResponse = yield axiosRequestWithBackoff({
                method: "post",
                url: swapTxUrl,
                data: {
            ...
            for (const [idx, tx] of allTransactions.entries()) {
                ...
                for (let sendAttempt = 0; sendAttempt < 3; sendAttempt++) {
                    try {
                        txId = yield connection.sendTransaction(tx, txOptions);
                        sendSuccess = true;
                        break;
                    }
                    catch (sendError) {
                ...
                const confirmed = yield confirmTransactionWithRetry(txId, blockhash, lastValidBlockHeight, 3);
                if (!confirmed) {
 ...
 * Returns an object { success, fee }.
 */
function swapSell(tokenAddress_1, tokenAmount_1) {
    return __awaiter(this, arguments, void 0, function* (tokenAddress, tokenAmount, priorityFeeMultiplier = 1, retryCount = 0, slippageOverride = null) {
        var _a, _b;
        ...
        try {
            ...
            const initialBalance = yield getTokenBalance(tokenAddress);
            if (initialBalance <= 0) {
            ...
            const sellSwapQuoteUrl = `${raydium_sdk_v2_1.API_URLS.SWAP_HOST}/compute/swap-base-in?inputMint=${tokenAddress}&outputMint=${spl_token_1.NATIVE_MINT}&amount=${amountToSell}&slippageBps=${effectiveSlippage * 100}&txVersion=V0`;
            const sellSwapQuoteResponse = yield axiosRequestWithBackoff({ method: "get", url: sellSwapQuoteUrl });
            ...
            const sellSwapTxUrl = `${raydium_sdk_v2_1.API_URLS.SWAP_HOST}/transaction/swap-base-in`;
            const sellSwapTxResponse = yield axiosRequestWithBackoff({
                method: "post",
                url: sellSwapTxUrl,
                data: {
                    computeUnitPriceMicroLamports,
            ...
            const allTxBuf = sellSwapTransactions.data.map((tx) => Buffer.from(tx.transaction, "base64"));
            const allTransactions = allTxBuf.map((txBuf) => isV0Tx ? web3_js_1.VersionedTransaction.deserialize(txBuf) : web3_js_1.Transaction.from(txBuf));
            for (const [idx, tx] of allTransactions.entries()) {
                tx.sign([owner]);
                ...
                const preTxBalance = yield getTokenBalance(tokenAddress);
                // Send with retries
                let txId = "";
                let sendSuccess = false;
                for (let sendAttempt = 0; sendAttempt < 4; sendAttempt++) { // More retries for sell
                    try {
                        ...
                        if (sendAttempt > 0) {
                            yield delay(3000); // Wait a bit to let the transaction propagate
                            const currentBalance = yield getTokenBalance(tokenAddress);
                ...
                if (!sendSuccess) {
                    ...
                    const finalCheckBalance = yield getTokenBalance(tokenAddress);
                ...
                const postTxBalance = yield getTokenBalance(tokenAddress);
                ...
                let confirmed = false;
                try {
                    confirmed = yield confirmTransactionWithRetry(txId, blockhash, lastValidBlockHeight, 5);
                }
                catch (confirmError) {
                    ...
                    const postConfirmBalance = yield getTokenBalance(tokenAddress);
                ...
                if (!confirmed) {
                    ...
                    const finalBalance = yield getTokenBalance(tokenAddress);
            ...
            if (retryCount < MAX_RETRIES) {
                ...
                const isExpiredTx = ((_a = error === null || error === void 0 ? void 0 : error.message) === null || _a === void 0 ? void 0 : _a.includes("TransactionExpiredBlockheightExceededError")) ||
                    ((_b = error === null || error === void 0 ? void 0 : error.message) === null || _b === void 0 ? void 0 : _b.includes("blockhash not found"));
 ...
 */
function sellInChunks(tokenAddress, totalAmount, feeMultiplier) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log(`[${new Date().toISOString()}] Attempting to sell in smaller chunks with high fees`);
        // Get current balance
        const currentBalance = yield getTokenBalance(tokenAddress);
        if (currentBalance <= 0)
            return { success: true, fee: feeMultiplier };
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            const remainingBalance = yield getTokenBalance(tokenAddress);
            ...
            try {
                const result = yield swapSell(tokenAddress, actualChunkAmount, chunkFeeMultiplier, 0, chunkSlippage);
                if (result.success) {
                    successCount++;
                    highestFeeUsed = Math.max(highestFeeUsed, result.fee);
                    yield delay(5000); // Wait before next chunk
                }
            }
            catch (error) {
 ...
 * Last resort emergency sell attempt with extreme parameters
 */
function emergencySellAttempt(tokenAddress, tokenAmount) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const results = yield Promise.all(attempts.map(p => p.catch(e => ({ success: false }))));
...
function getTokenPrice(tokenAddress) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const priceUrl = `${raydium_sdk_v2_1.API_URLS.SWAP_HOST}/compute/swap-base-out?inputMint=${tokenAddress}&outputMint=${spl_token_1.NATIVE_MINT}&amount=1000000&slippageBps=10`;
            // Retry up to 3 times with backoff
            let response;
            for (let attempt = 0; attempt < 3; attempt++) {
                try {
                    response = yield axiosRequestWithBackoff({ method: "get", url: priceUrl });
                    break;
                }
                catch (error) {
                    if (attempt === 2)
            ...
            if ((response === null || response === void 0 ? void 0 : response.data) && response.data.quotedAmountIn && response.data.quotedAmountOut) {
            ...
            try {
                ...
                const altPriceUrl = `${raydium_sdk_v2_1.API_URLS.SWAP_HOST}/compute/swap-base-in?inputMint=${spl_token_1.NATIVE_MINT}&outputMint=${tokenAddress}&amount=${0.01 * LAMPORTS_PER_SOL}&slippageBps=100`;
                const altResponse = yield axiosRequestWithBackoff({ method: "get", url: altPriceUrl });
                if ((altResponse === null || altResponse === void 0 ? void 0 : altResponse.data) && altResponse.data.quotedAmountIn && altResponse.data.quotedAmountOut) {
...
function getTokenBalance(tokenAddress) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                const ata = yield (0, spl_token_1.getAssociatedTokenAddress)(new web3_js_1.PublicKey(tokenAddress), owner.publicKey);
                const balanceInfo = yield connection.getTokenAccountBalance(ata);
...
function getSolBalance() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const balance = yield connection.getBalance(owner.publicKey);
...
*/
function getRecentFeesSpent() {
    return __awaiter(this, arguments, void 0, function* (lookbackSlots = 100) {
        try {
            // Get recent signatures for the wallet
            const signatures = yield connection.getSignaturesForAddress(owner.publicKey, { limit: 10 });
            if (!signatures.length)
                return 0;
            let totalFees = 0;
            for (const sigInfo of signatures) {
                const tx = yield connection.getTransaction(sigInfo.signature, {
            ...
            return totalFees / 1000000000; // Return in SOL
        }
        catch (error) {
